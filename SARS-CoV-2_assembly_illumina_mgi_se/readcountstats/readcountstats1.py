...
def get_cons2(bam, sample_number, gene, ref_file, step):
	...
	for i in ks:
		if rc[i]["COV"] < 10:
			print("Coverage is lower than 10 in position "+str(i)+". Please review basecount data.\n")
...
#Рисование графика покрытия
	...
	ppl.figure(figsize=(60,10), dpi=200)
	ppl.plot(ks, Cov, 'b', ks, cov_lim_arr, 'r--', ks, low_cov_lim_arr, 'y--')
	ppl.title(sample_name+" "+gene+", median coverage = "+str(m))
	ppl.yscale("log")
	ppl.yticks([1, 10, 100, 1000, 10000])
