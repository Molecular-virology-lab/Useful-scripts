...
def samtools_stats(bam_file):
    ...
    for l in out.decode().rstrip().split("\n"):
        ...
        if l[1] == 'total length:':
            total_l = int(l[2])
        elif l[1] == 'reads mapped:':
            mapped = int(l[2])
        elif l[1] == 'average length:':
            ave_length = int(l[2])

    return((total_l, mapped, ave_length))
...
def add_genes(ax):
    gene_tbl = pd.DataFrame(
        {
         'Name':["nsp1", "nsp2", "nsp3", "nsp4", "nsp5", "nsp6", "nsp7", "nsp8", "nsp9", "nsp10", "nsp12", "nsp13", "nsp14", "nsp15", "nsp16", "S", "ORF3a", "E", "M", "ORF6  ", "ORF7a  ", "      ORF7b", "      ORF8", "N", "ORF10"],
         'Start':[265,805,2719,8554,10054,10972,11842,12091,12685,13024,13441,16236,18039,19620,20658,21562,25392,26244,26522,27201,27393,27755,27893,28273,29557],
         'End':[804,2718,8553,10053,10971,11841,12090,12684,13023,13440,16235,18038,19619,20657,21555,25384,26220,26472,27191,27387,27759,27886,28259,29533,29674]
        }
                            )
...
def main(bam_files, outpdf, primer_bed=None, highlight_arg=None,
    ...
    for i in range(n_sample):
        ...
        meta_data = ['Total Seq: {:.1f} Mb'.format(align_stats[0]/1e6),
                     'Total reads: {:.1f}'.format(align_stats[1]),
                     'Average length: {:.1f}'.format(align_stats[2])]
    ...
    plt.savefig(outpdf+'.pdf', format='pdf')
    plt.savefig(outpdf+'.svg', format='svg')
    plt.savefig(outpdf+'.jpg', format='jpg')
    plt.savefig(outpdf+'.png', format='png')
